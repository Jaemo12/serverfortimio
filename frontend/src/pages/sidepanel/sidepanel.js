...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // initializeHeaderAnimation(); // Remove old call

  // Initialize resize functionality
  console.log('=== ABOUT TO CALL initializeResizeHandle ===');
  initializeResizeHandle();
  console.log('=== FINISHED CALLING initializeResizeHandle ===');

  // Store the current animation type globally
  let currentAnimationType = 'torch'; // Default to torch

  function setHeaderAnimation(type) {
    currentAnimationType = type;
    initializeHeaderAnimation(type);
  }
  ...
  function initializeHeaderAnimation(type = 'torch') {
    const headerAnimation = document.getElementById('header-animation');
    if (!headerAnimation) return;

    if (type === 'pivot') {
      // Use the clear Pivot symbol for Pivot
      headerAnimation.innerHTML = `
        <img src="assets/img/CustomPivotIcon.png" alt="Pivot Icon" style="width: 48px; height: 48px; display: block; margin: 0 auto;" />
      `;
      headerAnimation.style.animation = '';
    } else {
      // Default Torch/other: use SVG
      headerAnimation.innerHTML = `
  ...
  function initializeResizeHandle() {
    ...
    const MIN_WIDTH = 320;
    const MAX_WIDTH = 800;
    const STORAGE_KEY = 'timio_sidepanel_width';
    ...
    chrome.storage.local.get([STORAGE_KEY], (result) => {
      if (result[STORAGE_KEY]) {
        ...
        if (savedWidth >= MIN_WIDTH && savedWidth <= MAX_WIDTH) {
          applySidepanelWidth(savedWidth);
        }
      }
    });

    function applySidepanelWidth(width) {
      // Set the width using Chrome's sidePanel API if available
      if (chrome.sidePanel && chrome.sidePanel.setOptions) {
        chrome.sidePanel.setOptions({ width: width });
      }
      // Also store the width
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (request.type === 'SHOW_LOADING') {
      // Set header animation based on animationType
      setHeaderAnimation(request.animationType || 'torch');
      ...
      spinner.innerHTML = `
        <div class="timio-loading-container">
          <div class="timio-loading-header">
            ...
            <h3 class="timio-loading-title">${request.animationType === 'torch'
          ? 'Analyzing Article'
