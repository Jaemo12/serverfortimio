...
(() => {
    ...
    function formatInsights(insights) {
        ...
        insightContent = insightContent.replace(/\n/g, '\n').replace(/\"/g, '"').replace(/\\/g, '');

        // Torch section header (same format as Pivot, now with two lines)
        const header = `
          <div class="timio-section-header">
            <img src="${chrome.runtime.getURL('assets/img/CustomTorchIcon.png')}" class="timio-section-icon" alt="Torch Icon" />
            <div class="timio-section-header-text">
              <span class="timio-section-title">Torch</span>
              <span class="timio-section-subtext">Detect Bias With AI</span>
            </div>
          </div>
        `;

        // Split by **HEADER**
        // This regex will match all occurrences of **Header** and split accordingly
        const sectionRegex = /\*\*(.*?)\*\*/gs;
        let match;
        const sections = [];
        let lastIndex = 0;

        while ((match = sectionRegex.exec(insightContent)) !== null) {
          // match[1] is the header
          const header = match[1].trim();
          const start = match.index + match[0].length;
          // Find where the next header starts or end of string
          const nextMatch = sectionRegex.exec(insightContent);
          const end = nextMatch ? nextMatch.index : insightContent.length;
          // Reset regex lastIndex for next loop
          if (nextMatch) sectionRegex.lastIndex = nextMatch.index;
          const body = insightContent.slice(start, end).trim();
          // Split body into lines, filter out empty lines
          const points = body.split('\n').map(l => l.trim()).filter(l => l);
          sections.push({ header, points });
          // If nextMatch, continue; else break
          if (!nextMatch) break;
        }

        // If no sections found, fallback to old logic
        if (sections.length === 0) {
          const fallbackSections = insightContent.split('\n\n').filter(section => section.trim());
          return `
        ${header}
      ...
      `;
        ...
        // Render using new logic
        // Move 'Assessment' section to the top if it exists
        let assessmentIndex = sections.findIndex(section => section.header.toLowerCase() === 'assessment');
        let orderedSections = sections;
        if (assessmentIndex > -1) {
          const [assessmentSection] = sections.splice(assessmentIndex, 1);
          orderedSections = [assessmentSection, ...sections];
        }
        return `
      ${header}
      ${orderedSections.map(section => `
        <div class="timio-insight-section${section.header.toLowerCase() === 'assessment' ? ' assessment-card' : ''}">
          <h3 class="timio-insight-title${section.header.toLowerCase() === 'assessment' ? ' assessment-header' : ''}">${section.header}</h3>
          ${section.points.length > 0 ? `<ul class="timio-insight-list">\n          ${section.points.map(point => `\n            <li class="timio-insight-item">${point.replace(/^[•\-]\s*/, '').trim()}</li>\n          `).join('')}\n        </ul>` : ''}
        </div>
      `).join('')}
    `;
    }
    ...
    function formatPivotArticles(articles) {
        // Merged Pivot image fetching system for 1.5.1, using 1.4's fallback logic
        // This function is ready to drop into 1.5.1_0/content.bundle.js or similar
        ...
        // Add Pivot section header (from 1.5.1)
        const header = `
      <div class="timio-section-header">
        <img src="${chrome.runtime.getURL('assets/img/CustomPivotIcon.png')}" class="timio-section-icon" alt="Pivot Icon" />
        <div class="timio-section-header-text">
          <span class="timio-section-title">Pivot</span>
          <span class="timio-section-subtext">Read Different Views</span>
        </div>
      </div>
    `;
        ...
        timioModal.innerHTML = `
            <div class="timio-modal-content">
                <div class="timio-modal-header">
                    <div class="timio-modal-brand">
                        <img src="${chrome.runtime.getURL('assets/img/TIMIOLogoClear.png')}" alt="TIMIO Logo" />
                        <span>TIMIO News</span>
                        <button class="timio-modal-close">×</button>
                    </div>
                    <hr class="timio-modal-divider" />
                </div>
    ...
    function showLoadingInModal(animationType) {
        openModal();
        ...
        spinner.innerHTML = `
            <div class="timio-loading-column">
                ...
                ${animationType === 'torch' ? `
                <div class="timio-disclaimer">
                    <p>Torch can make mistakes. <br> Always verify with original sources.</p>
                </div>
                ` : ''}
            ...
            chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
                try {
                    ...
                    if (request.type === 'TOGGLE_FLOATING_MENU') {
                        ...
                        showLoadingInModal(request.animationType);
    ...
    function handleButtonClick(action, title, animationType) {
        return async () => {
            try {
                ...
                showLoadingInModal(animationType);
                ...
                <div class="timio-menu-container">
                    <div class="timio-menu-items">
                        <button class="timio-action-button" id="timio-insights">
                            ...
                            <span class="timio-tooltip">Bias Detection</span>
                        </button>
                        <button class="timio-action-button" id="timio-pivot">
                            <div class="button-image">
                                <img src="${pivotIconUrl}" alt="Pivot">
                            </div>
                            <span class="timio-tooltip">Read Different Views</span>
                        </button>
                    </div>
